...
export function SettingsPage() {
  ...
  return (
    <motion.div
      className="max-w-[780px] mx-auto px-8 py-8"
    ...
    >
      ...
      <div className="flex gap-1 bg-[#f1f5f9] p-1 rounded-2xl w-fit mb-8 shadow-inner">
        {tabs.map((t) => (
          <button
            ...
            className={`flex items-center gap-2 px-5 py-2.5 rounded-xl text-[13px] font-semibold transition-all relative ${
      ...
      <div className="bg-white border border-[#e2e8f0] rounded-2xl overflow-hidden shadow-sm">
        <AnimatePresence mode="wait">
          {tab === 'profile' && (
            <motion.div
              ...
              className="p-7"
            >
              <h2 className="text-[15px] font-bold text-[#0f172a] mb-6">Profile Information</h2>

              {/* Avatar section */}
              <div className="flex items-center gap-5 mb-7 p-5 bg-[#f8fafc] rounded-2xl border border-[#f1f5f9]">
                <div className="relative flex-shrink-0">
                  <div className="w-16 h-16 rounded-2xl bg-gradient-to-br from-[#0ea5e9] to-[#0284c7] flex items-center justify-center text-white text-xl font-bold shadow-lg shadow-sky-200">
                    {user ? getInitials(user.full_name || user.email) : '?'}
                  </div>
                  <button className="absolute -bottom-1 -right-1 w-6 h-6 bg-white border-2 border-[#f1f5f9] rounded-full flex items-center justify-center text-[#64748b] hover:bg-[#f1f5f9] shadow-md transition-colors">
                    <Camera className="w-3 h-3" />
                  </button>
                </div>
                <div className="flex-1">
                  <p className="text-[15px] font-bold text-[#0f172a] leading-tight">{user?.full_name || user?.email}</p>
              ...
              <div className="space-y-5">
                <div>
                  ...
                  <input
                    ...
                    className="w-full bg-[#f8fafc] border border-[#e2e8f0] rounded-xl px-4 py-3 text-[14px] text-[#0f172a] outline-none focus:border-[#0ea5e9]/50 focus:bg-white transition-all"
                ...
                <div>
                  ...
                  <input
                    ...
                    value={user?.email || ''}
                    disabled
                    className="w-full bg-[#f8fafc] border border-[#f1f5f9] rounded-xl px-4 py-3 text-[14px] text-[#94a3b8] cursor-not-allowed"
                  />
            ...
            >
              ...
              <div className="space-y-1">
                {[
                  ...
                  <div
                    key={item.key}
                    className="flex items-center justify-between py-4 border-b border-[#f8fafc] last:border-0 group"
                  >
                    <div>
        ...
        {/* Footer */}
        <div className="px-7 py-4 bg-[#f8fafc] border-t border-[#f1f5f9] flex justify-end">
          <motion.button
