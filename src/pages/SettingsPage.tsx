  ...
  return (
    ...
    >
      ...
      <div className="bg-white border border-[#e2e8f0] rounded-2xl overflow-hidden">
          ...
          <div className="p-6">
            ...
            <div className="flex items-center gap-5 mb-6 pb-6 border-b border-[#f1f5f9]">
              <div className="relative">
                ...
                <button className="absolute -bottom-1 -right-1 w-6 h-6 bg-white border border-[#e2e8f0] rounded-full flex items-center justify-center text-[#64748b] hover:bg-[#f1f5f9] transition-colors shadow-sm">
                  <Camera className="w-3 h-3" />
                </button>
              </div>
              <div>
                <p className="text-[14px] font-medium text-[#0f172a]">{user?.full_name || 'User'}</p>
                <p className="text-[12px] text-[#94a3b8]">{user?.email}</p>
                <p className="text-[11px] text-[#94a3b8] mt-1 capitalize">
                  Role: <span className="text-[#64748b]">{user?.role}</span>
                </p>
              </div>
            </div>
          ...
          <div className="p-6">
            ...
            <div className="space-y-1">
              {[
                ...
                { key: 'dailyDigest', label: 'Daily Digest', desc: 'A daily summary of activity (sent at 8am)' },
                  ...
                  >
                    <div
                      className={`absolute top-0.5 w-[18px] h-[18px] rounded-full bg-white shadow-sm transition-all ${
                        notifs[item.key as keyof typeof notifs] ? 'left-[19px]' : 'left-0.5'
                      }`}
                    />
                  </button>
                </div>
        ...
        {tab === 'security' && (
          <div className="p-6">
            <h2 className="text-[15px] font-semibold text-[#0f172a] mb-5">Security</h2>
            <div className="space-y-4">
              <div>
                <label className="block text-[11px] font-semibold text-[#94a3b8] uppercase tracking-wider mb-2">
                  Current Password
